...
function canvas(canvasId){
    ...
    this.view = function(type){
        ...
        if(type === "menu"){
            ...
            aimCanvas.ctx.arc(this.centerLeft - 45, this.centerTop - 50, 10, 0, 2 * Math.PI);
            aimCanvas.ctx.closePath();
            aimCanvas.ctx.fill();

            aimCanvas.ctx.beginPath();
            aimCanvas.ctx.arc(this.centerLeft - 10, this.centerTop - 10, 20, 0, 2 * Math.PI);
            aimCanvas.ctx.closePath();
            aimCanvas.ctx.fill();

            aimCanvas.ctx.beginPath();
            aimCanvas.ctx.arc(this.centerLeft - 25, this.centerTop + 25, 5, 0, 2 * Math.PI);
            aimCanvas.ctx.closePath();
            aimCanvas.ctx.fill();

            aimCanvas.ctx.beginPath();
            aimCanvas.ctx.arc(this.centerLeft - 45, this.centerTop + 45, 10, 0, 2 * Math.PI);
            aimCanvas.ctx.closePath();
            aimCanvas.ctx.fill();
            ...
            aimCanvas.ctx.beginPath();
            aimCanvas.ctx.arc(this.centerLeft + 45, this.centerTop -30 , 15, 0, 2 * Math.PI);
            aimCanvas.ctx.closePath();
            aimCanvas.ctx.fill();

            aimCanvas.ctx.beginPath();
            aimCanvas.ctx.arc(this.centerLeft + 30, this.centerTop + 30, 20, 0, 2 * Math.PI);
            aimCanvas.ctx.closePath();
            aimCanvas.ctx.fill();

            aimCanvas.ctx.fillStyle = "#eb645d";
            aimCanvas.ctx.textAlign = "center";
            aimCanvas.ctx.textBaseline = "center";
            aimCanvas.ctx.font = "bold 50px Gilroy";
            aimCanvas.ctx.fillText("LET'S", this.centerLeft, this.centerTop + 0);
			aimCanvas.ctx.font = "30px Gilroy";
				aimCanvas.ctx.fillStyle = "#f6b949";
                aimCanvas.ctx.fillText("TRAIN YOR AIM", this.centerLeft, this.centerTop + 40);
            ...
            if(this.mode.life === 0){
                ...
                aimCanvas.ctx.textBaseline = "center";
                aimCanvas.ctx.font = "bold 50px Gilroy";
                aimCanvas.ctx.fillText("GAME OVER", this.centerLeft, this.centerTop - 20);
                aimCanvas.ctx.font = "30px Gilroy";
				aimCanvas.ctx.fillStyle = "#f6b949";
                aimCanvas.ctx.fillText("SCORE : " + this.mode.score, this.centerLeft, this.centerTop + 20);
				aimCanvas.ctx.fillStyle = "#404040";
                aimCanvas.ctx.fillText("PRESS ESCAPE", this.centerLeft, this.centerTop + 60);

            }else{

                this.ctx.fillStyle = "#603030";
                this.ctx.textAlign = "center";
                this.ctx.textBaseline = "center";
                this.ctx.font = "80px Gilroy";
                this.ctx.fillText('♥'.repeat(this.mode.life), this.centerLeft, this.centerTop - 5);

                aimCanvas.ctx.font = "30px Gilroy";
				this.ctx.fillStyle = "#604030";
                aimCanvas.ctx.fillText("SCORE : " + this.mode.score, this.centerLeft, this.centerTop + 25);
				aimCanvas.ctx.fillStyle = "#404040";
                aimCanvas.ctx.fillText("MISS : " + this.mode.shootFail, this.centerLeft, this.centerTop + 65);
...
function targetMode(){
    ...
    this.addTarget = function(){
		...
		this.targetsTime = 1000 - (this.score * 8); // Unity : Mills
		
		if(this.targetsTime < -3000){

            this.targetsRapidity = 0.5; // Unity : Pixel

		}else if(this.targetsTime < -1400){
			
			this.targetsRapidity = 0.35; // Unity : Pixel

		}else if(this.targetsTime < -40){
			
			this.targetsRapidity = 0.25; // Unity : Pixel
        ...
        if(this.targets.length < 5 && (Date.now() + 1) > this.targetsLastAdd + this.targetsTime){
...
function target(){
    ...
    this.draw = function(){
        ...
        aimCanvas.ctx.closePath();
        aimCanvas.ctx.fill();
		aimCanvas.ctx.fillStyle = "#fff";
        aimCanvas.ctx.beginPath();
        aimCanvas.ctx.arc(this.x, this.y, this.size / 1.2, 0, 2 * Math.PI);
        aimCanvas.ctx.closePath();
        aimCanvas.ctx.fill();
		aimCanvas.ctx.fillStyle = "#eb645d";
        aimCanvas.ctx.beginPath();
		...
		aimCanvas.ctx.fillStyle = "#fff";
        aimCanvas.ctx.beginPath();
        aimCanvas.ctx.arc(this.x, this.y, this.size / 2, 0, 2 * Math.PI);
        aimCanvas.ctx.closePath();
        aimCanvas.ctx.fill();
		aimCanvas.ctx.fillStyle = "#eb645d";
        aimCanvas.ctx.beginPath();
        aimCanvas.ctx.arc(this.x, this.y, this.size / 3, 0, 2 * Math.PI);
        aimCanvas.ctx.closePath();
        aimCanvas.ctx.fill();
...
function sound(){
    ...
    this.sound.src = "/app/modules/module_page_aimka/assets/sound/shoot.mp3";
